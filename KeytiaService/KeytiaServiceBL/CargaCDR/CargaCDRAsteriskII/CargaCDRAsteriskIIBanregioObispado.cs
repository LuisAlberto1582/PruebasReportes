...
Nombre:		    DDCP
Fecha:		    20110519
Descripción:	Clase con la lógica standar para los conmutadores Asterisk II del Cliente BanRegio sitio Obispado
Modificación:	
*/
using System;
...
{
    public class CargaCDRAsteriskIIBanregioObispado : CargaCDRAsteriskIIBanregio
    {
        public CargaCDRAsteriskIIBanregioObispado()
        {
            piColumnas = 11;
            piSRC = 0;
            piDST = 1;
        ...
        protected override void ActualizarCamposSitio()
        {
            ...
            if (lAsCDR[piDST].Trim().Length == 3 && lAsCDR[piDST].Trim().Substring(0, 1) != "*" && lAsCDR[piSRC].Trim().Length != 4 && lAsCDR[piDstChannel].Trim().Length >= 11 && lAsCDR[piDstChannel].Trim().Substring(0, 5) == "local")
            {
                psCDR[piSRC] = lAsCDR[piDstChannel].Trim().Substring(6, 4);
                lAsCDR = (string[])psCDR.Clone();
            }

            if (lAsCDR[piDST].Trim().Length == 3 && lAsCDR[piDST].Trim().Substring(0, 1) != "*" && lAsCDR[piSRC].Trim().Length != 4 && lAsCDR[piDstChannel].Trim().Length >= 9 && lAsCDR[piDstChannel].Trim().Substring(0, 3) == "SIP")
            {
