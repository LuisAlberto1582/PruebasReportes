...
namespace KeytiaServiceBL.CargaCDR.CargaCDRCisco
{
    ...
    public class CargaCDRCiscoKuehneNagel131Col : CargaCDRCiscoKuehneNagel
    {
        public int piOriginalCalledPartyNumber { get; set; }

        protected StringBuilder sbDIDs = new StringBuilder();
        ...
        protected override string GetEtiqueta()
        {
            ...
            if (psCDR != null)
            {
                if (!string.IsNullOrEmpty(psCDR[piDIDEntrada].ToString()))
                {
                    sbDIDs.Append(psCDR[piDIDEntrada].ToString());
                }

                if (!string.IsNullOrEmpty(psCDR[piDIDSalida].ToString()))
                {
                    if (sbDIDs.Length == 0)
                        sbDIDs.Append(psCDR[piDIDSalida].ToString());
                    else
                        sbDIDs.Append(" - " + psCDR[piDIDSalida].ToString());
                }
            }
        ...
        public CargaCDRCiscoKuehneNagel131Col()
        {
            piColumnas = 131;
