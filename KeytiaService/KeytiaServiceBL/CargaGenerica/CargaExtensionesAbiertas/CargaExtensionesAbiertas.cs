...
namespace KeytiaServiceBL.CargaGenerica.CargaExtensionesAbiertas
{
    public class CargaExtensionesAbiertas : CargaServicioGenerica
    {
        List<ExtensionesAbiertas> listExtensiones = new List<ExtensionesAbiertas>();

        int icodCarga;
        string mesCod;
        string anioCod;

        public CargaExtensionesAbiertas()
        {
        ...
        public override void IniciarCarga()
        {
            ...
            pfrXLS.CambiarHoja("Extensiones abiertas");
        ...
        {
            ExtensionesAbiertas list = new ExtensionesAbiertas();
            ...
            list.Extension = psaRegistro[11].Trim();
            list.Permiso = Convert.ToInt32(psaRegistro[12].Trim());
        ...
        {
            string sp = "EXEC dbo.WorkflowBanorteInsertEventosIndependientes @NominaEmpleado = {0}, @Solicitante = {1}, @SolicitanteEmail = {2}, @NominaJefe = {3}, @JefeEmail = {4}, " +
                        "@Recurso = {5}, @TipoMovimiento = {6}, @SitioID = {7}, @Extension = {8}, @PermisoCod = {9}";
            ...
            foreach (var item in listExtensiones)
            {
                string query = string.Format(sp, item.NumeroEmpleado, item.NombreSolicitante, item.EmailSolicitante, item.NominaJefe, item.EmailJefe, 
                                            item.Recurso, item.Movimiento, item.SiteID, item.Extension, item.Permiso);
                
                DSODataAccess.ExecuteNonQuery(query);
            }
        }
        public class ExtensionesAbiertas
        {
