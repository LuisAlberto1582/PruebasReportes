...
namespace KeytiaServiceBL.CargaFacturas.CargaFacturaTotalPlayTIM
{
    ...
    public class CargaFacturaTotalPlayTIM : KeytiaServiceBL.CargaFacturas.CargaServicioFactura
    {
        protected StringBuilder query = new StringBuilder();
        protected List<FileInfo> archivos = new List<FileInfo>();
        protected List<TIMDetalleFacturaTotalPlay> listaDetalleFactura = new List<TIMDetalleFacturaTotalPlay>();
        ...
        public CargaFacturaTotalPlayTIM()
        {
            pfrXLS = new FileReaderXLS();

            carrier = "TotalPlay";
            vchDescMaestro = "Cargas Factura TotalPlay TIM";
            nombreConsolidadoPendientes = "Consolidado de Carga TotalPlay TIM";
            nombreTablaIndividualDetalle = DiccVarConf.TIMTablaTIMTotalPlayDetalleFactura;
        }
        
        public virtual void GenerarConsolidadoPorClaveCargo()
        {
            DSODataAccess.ExecuteNonQuery("EXEC [TIMTotalPlayGeneraConsolidadoPorClaveCar] @Esquema = '" + DSODataContext.Schema + "', @iCodCatCarga = " + CodCarga);
        }

        public virtual void GenerarConsolidadoPorSitio()
        {
            DSODataAccess.ExecuteNonQuery("EXEC [TIMTotalPlayGeneraConsolidadoPorSitio] @Esquema = '" + DSODataContext.Schema + "', @iCodCatCarga = " + CodCarga);
        }
                ...
                if (archivos[i].Name.ToLower() == @numCuentaMaestra + "_detallefactura_" + @fechaInt + archivos[i].Extension.ToLower())
                {
                    ...
                    if (!
                        (
                            ...
                            psaRegistro[12].ToLower().Replace(" ", "") == "subcuenta" 
                        )
                          )
                    {
                        ActualizarEstCarga("ArchNoVal" + (i + 1).ToString(), psDescMaeCarga);
        ...
        public virtual bool VaciarInfoDetalleFactura(int indexArchivo)
        {
            try
            {
                ...
                while ((psaRegistro = pfrXLS.SiguienteRegistro()) != null)
                {
                    ...
                    {
                        TIMDetalleFacturaTotalPlay detall = new TIMDetalleFacturaTotalPlay();
                        ...
                        int.TryParse(psaRegistro[8].Trim(), out minutos);
                        int.TryParse(psaRegistro[9].Trim(), out llamadas);
                        ...
                        detall.Subcuenta = psaRegistro[12].ToString();
        ...
        public virtual void InsertarDetalleFactura()
        {
            try
            {
                if (listaDetalleFactura.Count > 0)
                {
                    ...
                    query.AppendLine("CostoMonLoc,Llamadas, Minutos,Velocidad,IDSitio, Subcuenta, dtFecUltAct)");
                    query.Append("VALUES ");

                    foreach (TIMDetalleFacturaTotalPlay item in listaDetalleFactura)
                    {
                        ...
                        query.AppendLine((string.IsNullOrEmpty(item.Subcuenta)) ? "NULL, " : "'" + item.Subcuenta + "', ");
                        query.AppendLine("GETDATE()),");
        ...
        {
            TIMDetalleFacturaTotalPlay item = (TIMDetalleFacturaTotalPlay)listaDetalleFactura[0];
        ...
        //private bool VerificarExisteCtaMaestra(List<TIMDetalleFacturaTotalPlay> listaDetalleFactura)
        //{
        //    StringBuilder query = new StringBuilder();
        //    query.Append("Select * ");
