using KeytiaServiceBL.CargaFacturas.CargaFacturaAxtelV3TIM;
...
namespace KeytiaServiceBL.CargaFacturas.CargaFacturaAxtelV2AlestraTIM
{
    public class CargaFacturaAxtelV3TIM : CargaServicioFactura
    {
        protected StringBuilder query = new StringBuilder();
        protected List<FileInfo> archivos = new List<FileInfo>();
        protected List<TIMDetalleFacturaAxtelV3> listaDetalleFactura = new List<TIMDetalleFacturaAxtelV3>();
        ...
        public override void IniciarCarga()
        {
            ...
            piRegistro = this.listaDetalleFactura.Count;
                ...
                if (archivos[i].Name.ToLower() == @numCuentaMaestra + "_detallefactura_" + @fechaInt + archivos[i].Extension.ToLower())
                {
                    ...
                    if (!
                        (
                            ...
                            psaRegistro[8].ToLower().Replace(" ", "") == "ctadeservicio" &&
                            psaRegistro[9].ToLower().Replace(" ", "") == "llave_servicio" &&
                            psaRegistro[10].ToLower().Replace(" ", "") == "minutos" &&
                            psaRegistro[11].ToLower().Replace(" ", "") == "llamadas" &&
                            psaRegistro[12].ToLower().Replace(" ", "") == "velocidad" &&
                            psaRegistro[13].ToLower().Replace(" ", "") == "idsitio"
                        )
                          )
                    {
                        ActualizarEstCarga("ArchNoVal" + (i + 1).ToString(), psDescMaeCarga);
        ...
        public virtual bool VaciarInfoDetalleFactura(int indexArchivo)
        {
            try
            {
                ...
                while ((psaRegistro = pfrXLS.SiguienteRegistro()) != null)
                {
                    ...
                    {
                        TIMDetalleFacturaAxtelV3 detall = new TIMDetalleFacturaAxtelV3();
                        ...
                        detall.CuentaServicio = psaRegistro[8].Trim();
                        detall.LlaveServicio = psaRegistro[9].Trim();
                        ...
                        int.TryParse(psaRegistro[10].Trim(), out minutos);
                        int.TryParse(psaRegistro[11].Trim(), out llamadas);

                        detall.Llamadas = llamadas;
                        detall.Minutos = minutos;
                        detall.Velocidad = psaRegistro[12].ToString();
                        detall.IdSitio = psaRegistro[13].ToString();
                        ...
                        this.listaDetalleFactura.Add(detall);
                    }
                }
        ...
        public virtual void InsertarDetalleFactura()
        {
            try
            {
                if (listaDetalleFactura.Count > 0)
                {
                    ...
                    query.AppendLine("CostoMonLoc,Llamadas, Minutos,Velocidad,IDSitio, CuentaServicio, LlaveServicio, dtFecUltAct)");
                    query.Append("VALUES ");

                    foreach (TIMDetalleFacturaAxtelV3 item in listaDetalleFactura)
                    {
                        ...
                        //FD 20210422 Cambio Banregio: Se añaden columnas CuentaServicio y Llave Servicio
                        query.Append((string.IsNullOrEmpty(item.CuentaServicio)) ? "NULL, " : "'" + item.CuentaServicio + "', ");
                        query.Append((string.IsNullOrEmpty(item.LlaveServicio)) ? "NULL, " : "'" + item.LlaveServicio + "', ");
                        query.AppendLine("GETDATE()),");
        ...
        public override bool EliminarCarga(int iCodCatCarga)
        {
            ...
            {
                new string[]{"[VisPendientes('Detall','" + nombreConsolidadoPendientes+ "')]", "iCodCatalogo"},
        ...
        {
            TIMDetalleFacturaAxtelV3 item = (TIMDetalleFacturaAxtelV3)listaDetalleFactura[0];
        ...
        private bool VerificarExisteCtaMaestra(List<TIMDetalleFacturaAxtelV3> listaDetalleFactura)
        {
            StringBuilder query = new StringBuilder();
            query.Append("Select * ");
            ...
            else
            {
                ...
                {
                    if (!Cuentas.AsEnumerable().Any(x => x.Field<string>("CuentaServicio") == item.CuentaServicio
                    && x.Field<int>("CtaMae") == item.ICodCatCtaMaestra))
                    {
                        listaLogPendiente.Add(string.Format(DiccMens.TIM0037, item.CuentaServicio, item.Sitio, carrier));
                        result = false;
                    }
                }
            }
