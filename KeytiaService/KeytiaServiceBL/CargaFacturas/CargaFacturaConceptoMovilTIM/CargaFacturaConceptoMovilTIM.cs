...
namespace KeytiaServiceBL.CargaFacturas.CargaFacturaConceptoMovilTIM
{
    public class CargaFacturaConceptoMovilTIM : KeytiaServiceBL.CargaFacturas.CargaFacturaAlestraTIM.CargaFacturaAlestraTIM
    {
        public CargaFacturaConceptoMovilTIM()
        {
            pfrXLS = new FileReaderXLS();

            carrier = "ConceptoMovil";
            vchDescMaestro = "Cargas Factura ConceptoMovil TIM";
            nombreConsolidadoPendientes = "Consolidado de Carga ConceptoMovil TIM";
            nombreTablaIndividualDetalle = DiccVarConf.TIMTablaTIMConceptoMovilDetalleFactura;
        }

        public override void GenerarConsolidadoPorClaveCargo()
        {
            DSODataAccess.ExecuteNonQuery("EXEC [TIMConceptoMovilGeneraConsolidadoPorClaveCar] @Esquema = '" + DSODataContext.Schema + "', @iCodCatCarga = " + CodCarga);
        }

        public override void GenerarConsolidadoPorSitio()
        {
            DSODataAccess.ExecuteNonQuery("EXEC [TIMConceptoMovilGeneraConsolidadoPorSitio] @Esquema = '" + DSODataContext.Schema + "', @iCodCatCarga = " + CodCarga);
        }

        public void GeneraTarifasGlobales()
        {
            TIMDetalleFacturaConceptoMovil item = (TIMDetalleFacturaConceptoMovil)listaDetalleFactura[0];
        ...
        public void GeneraMatrizMensajes()
        {
            try
            {
                ...
                {
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("Exec TIMConceptoMovilGeneraMatrizMensajes	");
        ...
        public override void InsertarDetalleFactura()
        {
            try
            {
                if (listaDetalleFactura.Count > 0)
                {
                    ...
                    foreach (TIMDetalleFacturaConceptoMovil item in listaDetalleFactura)
                    {
            ...
            try
            {
                ...
                while ((psaRegistro = pfrXLS.SiguienteRegistro()) != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(psaRegistro[0].Trim()))
                    {
                        ...
                        TIMDetalleFacturaConceptoMovil detall = new TIMDetalleFacturaConceptoMovil();
