...
var jqWindowsEngineZIndex = 10000;

(function($) {
    $.fn.extend({
        $getWindowContainer: function() {
            return $(this).closest(".window-container");
        },
        newWindow: function(opts) {
            ...
            if (options.textOptions != null && $(options.textOptions).val() !== "") {
                try {
                    currentValues = JSON.parse($(options.textOptions).val());
                }
                catch (e) {
                    currentValues = {};
                }
            }
            options = $.extend(options, currentValues)

            var $modalBackground = $('<div class="modal-background"></div>');
            $modalBackground.css("z-index", jqWindowsEngineZIndex++);
            var $windowContainer = $('<div class="window-container"></div>');
            ...
            var $windowContent = $(this); //$('<div class="window-content"></div>');
            ...
            if (options.textOptions != null)
                $windowContainer.attr("textOptions", options.textOptions);

            if (options.type === "iframe") {
                $windowContent.css("overflow", "hidden");
            }

            if (options.onAjaxContentLoaded !== null) {
                $windowContainer.attr("textOptions", options.onAjaxContentLoaded);
            }
            ...
            var resize = function($obj, width, height) {
                ...
                updateTextOptions({ width: width, height: height });
            ...
            var updateTextOptions = function(values) {
                if (options.textOptions) {
                    var currentValues;
                    try {
                        currentValues = JSON.parse($(options.textOptions).val());
                    } catch (e) { currentValues = {}; }
                    currentValues = $.extend(currentValues, values);
                    $(options.textOptions).val(JSON.stringify(currentValues));
                }
            }
            ...
            var move = function($obj, x, y) {
                x = parseInt(x);
                y = parseInt(y);
                updateTextOptions({ posx: x, posy: y });
            ...
            $windowMinimizeButton.bind('click', function(e) {
                ...
                if ($obj.data("state") === "minimized") {
                    ...
                    $obj.find(".window-content").slideDown("slow");
                }
                else if ($obj.data("state") === "normal") {
                    $obj.data("state", "minimized");
                    //$obj.find(".window-content").slideToggle("slow", function() { $obj.css("height", 0); });
                    $obj.find(".window-content").slideUp("slow", function() { $obj.css("height", 0); });
                }
                else {
                    $obj.find(".window-maximizeButton").click();
                }
                updateTextOptions({ state: $obj.data("state") });
            });
            ...
            $windowCloseButton.bind('click', function(e) {
                ...
                $window.fadeOut(function() {
                    var $windowContent = $window.find(".window-content").detach();
                    $windowContent.css("display", "none");
                    $("form").append($windowContent);
                    $window.remove();
                });
            ...
            $window.data("lastY", options.posy)
                   .data("lastX", options.posx)
                   .data("lastWidth", options.width)
                   .data("lastHeight", options.height);
            ...
            if (options.display === undefined || options.display == false) {
                this.hideWindow();
            }
            else {
        ...
        updateWindowContent: function(newContent) {
            var $windowContainer = this.$getWindowContainer();
            $windowContainer.find(".window-content").html(newContent);
        },

        updateWindowContentWithAjax: function(url, cache) {
            var $windowContainer = this.$getWindowContainer();
            ...
            $.ajax({
                ...
                success: function(data) {
                    $(this).html(data);
                    if ($(this).attr("onAjaxContentLoaded") !== null) {
                        var onAjaxContentLoaded = (typeof $(this).attr("onAjaxContentLoaded")) == 'string' ?
                                                eval('(' + $(this).attr("onAjaxContentLoaded") + ')') :
                                                $(this).attr("onAjaxContentLoaded");
                        $(this).bind("onAjaxContentLoaded", onAjaxContentLoaded);
                        $(this).trigger("onAjaxContentLoaded");
                    }
                }
            });
        },
        ...
        moveWindow: function(x, y) {
            var $obj = this.$getWindowContainer();
            x = parseInt(x);
            y = parseInt(y);

            this.updateTextOptions({ posx: x, posy: y })
        ...
        resizeWindow: function(width, height) {
            ...
            this.updateTextOptions({ width: width, height: height })
        ...
        minimizeWindow: function() {
            var $windowContainer = this.$getWindowContainer();
            $windowContainer.find(".window-minimizeButton").click();
        },
        ...
        showWindow: function() {
            ...
            this.updateTextOptions({ display: true })
        },

        hideWindow: function() {
            var $windowContainer = this.$getWindowContainer();
            $windowContainer.fadeOut();
            this.updateTextOptions({ display: false })
        },

        closeWindow: function() {
            var $windowContainer = this.$getWindowContainer();
            $windowContainer.find(".window-closeButton").click();
        },

        closeAllWindows: function() {
            $(".window-container .window-closeButton").click();
        },
        updateTextOptions: function(values) {
            var $windowContainer = this.$getWindowContainer();
            if ($windowContainer.attr("textOptions")) {
                var currentValues;
                try {
                    currentValues = JSON.parse($($windowContainer.attr("textOptions")).val());
                } catch (e) { currentValues = {}; }
                currentValues = $.extend(currentValues, values);
                $($windowContainer.attr("textOptions")).val(JSON.stringify(currentValues));
            }
        }
